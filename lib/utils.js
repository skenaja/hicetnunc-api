...
async function getIpfsHash(ipfsHash) {
  console.log("getIpfsHash", ipfsHash)
  try {
    const i = (await axios.get('https://cloudflare-ipfs.com/ipfs/' + ipfsHash)).data
    return i
  } catch(e) {
    console.log(e)
  }
...
async function getObjektByIdBCD(id) {
  console.log("getObjektByIdBCD", id)
...
async function getObjktById(id, with_swaps=true) {
  console.log("getObjktById", id, with_swaps)
  ...
  console.log(id_int, block_range, block_start, block_end)
...
try {
  console.log("getObjktOwners", objkt)
  const owners = (
    await axios.get(
